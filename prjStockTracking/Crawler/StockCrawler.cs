...
namespace prjStockTracking.Crawler
{
    ...
    {
        // 1.取得黃金交叉清單
        List<Gold> Get_GoldenCrossList(string market);
        // 2.取得個股本淨比資料
        Pbr Get_PbrFlow(string STOCK_ID, string CHT_CAT);
        // 3.取得個股本益比資料
        Per Get_PerFlow(string STOCK_ID, string CHT_CAT);
        // 4.關閉連接(Client or Driver)
        void Dispose();
    }

    public class StockCrawler_Selenium : IStockCrawler
    {
        private HtmlDocument doc = new HtmlDocument();
        private MemoryStream ms = new MemoryStream();
        private IWebDriver driver;
        ...
        //非靜態方法，使用需先new()出物件，會啟動建構子
        public StockCrawler_Selenium()
        {
            driver = new ChromeDriver();
        }

        // Selenium
        // 1.取得黃金交叉清單(鉅亨網--黃金交叉的個股清單) //Selenium+HtmlDocument
        public List<Gold> Get_GoldenCrossList(string market) //**參數: 市場別(TSE/OTC)
        {
            List<Gold> goldList = new List<Gold>();
        ...
        // 2.取得個股本淨比資料(GoodInfo--個股本淨比河流)  //Selenium+HtmlDocument
        public Pbr Get_PbrFlow(string STOCK_ID, string CHT_CAT) //Selenium+HtmlDocument
        {
            string url = "https://goodinfo.tw/tw/";
            string new_url = url + "ShowK_ChartFlow.asp?RPT_CAT=PBR&STOCK_ID=" + STOCK_ID + "&CHT_CAT=" + CHT_CAT;
        ...
        // 3.取得個股本益比資料(GoodInfo--個股本益比河流)  //Selenium+HtmlDocument
        public Per Get_PerFlow(string STOCK_ID, string CHT_CAT) //Selenium+HtmlDocument
        {
            //if (STOCK_ID.Length != 4)
            //{
            //    return null;
    ...
    public class StockCrawler_WebClient : IStockCrawler
    {
        private WebClient _client;
        private HtmlDocument doc = new HtmlDocument();
        private MemoryStream ms = new MemoryStream();
        ...
        //非靜態方法，使用需先new()出物件，會啟動建構子
        public StockCrawler_WebClient(WebClient client)
        {
            _client = client;
            _client.Headers.Add("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36)");
        }

        // 1.取得黃金交叉清單 //webclient + HtmlDocument
        public List<Gold> Get_GoldenCrossList(string market)
        {
            ...
            //將網頁來源資料暫存到記憶體內
            ms = new MemoryStream(_client.DownloadData(url));
            //檢查資料
            string str = System.Text.Encoding.Default.GetString(ms.ToArray());
            //HtmlDocument doc = new HtmlDocument();
            doc.Load(ms, Encoding.UTF8); // 使用 UTF8 編碼讀入 HTML
            ...
            for (int tb_count = 1; tb_count < 6; tb_count++)
            {
                ...
                if (tb_var != null)
                {
                    ...
                    do
                    {
                        //Xpath：透過 Webclient取得的要去掉tbody，但 IWebDriver.PageSource則要保留tbody
                        string tr_xpath = xpath + "/tbody/tr[" + tr_count + "]";
                        tr_xpath = tr_xpath.Replace("/tbody", "");
                        tr_var = doc.DocumentNode.SelectNodes(tr_xpath);
        ...
        // 2.取得個股本淨比資料  //webclient + HtmlDocument
        public Pbr Get_PbrFlow(string STOCK_ID, string CHT_CAT)
        {
            if (STOCK_ID.Length != 4)
            {
        ...
        // 3.取得個股本益比資料  //webclient + HtmlDocument
        public Per Get_PerFlow(string STOCK_ID, string CHT_CAT)
        {
            if (STOCK_ID.Length != 4)
            {
